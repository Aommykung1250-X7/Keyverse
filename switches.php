...
// --- ส่วนจัดการ Filter ---
$filter = $_GET['filter'] ?? 'all'; // รับค่า filter จาก URL, default = 'all'
$sql_condition = ""; // เงื่อนไข SQL เริ่มต้น

// กำหนด Sub-Category ที่เป็นไปได้สำหรับ Switches
$switch_sub_categories = ['Linear', 'Tactile', 'Clicky', 'Low Profile'];

// สร้างเงื่อนไข SQL ตาม Filter ที่เลือก
if ($filter !== 'all' && in_array($filter, $switch_sub_categories)) {
...
<head>
  ...
  <title>Switches - KEYVERSE</title> 
  
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css" />

  <style>
    /* (CSS เหมือน keycap.php ... Navbar, Filters, Product Grid) */
    ...
    .product-card img { width: 100%; height: 190px; object-fit: cover; } /* อาจจะปรับ object-fit เป็น contain ถ้าต้องการ */
...
<body>
    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse d-lg-flex" id="navbarsExample11">
        ...
        <ul class="navbar-nav col-lg-6 justify-content-lg-center">
          ...
          <li class="nav-item">
            <a class="nav-link active" href="switches.php">Switches</a> </li>
          <li class="nav-item">
  ...
  <div class="section" style="width: 100%; padding: 15px 45px 15px 45px;">
    <h2>Switches</h2>
    <p class="subtitle">
        หัวใจสำคัญของ Mechanical Keyboard คือสวิตช์ เลือกสัมผัสและเสียงที่ใช่สำหรับคุณ<br/>
        ไม่ว่าจะเป็น Linear นุ่มลื่น, Tactile จังหวะมั่นคง, หรือ Clicky เสียงเร้าใจ
    </p>
  </div>
  <div class="d-flex flex-wrap justify-content-between align-items-center py-2 my-1 border-top "></div>

  <div class="filter-button-container mt-4">
    <a href="switches.php" class="filter-btn <?php echo ($filter === 'all') ? 'active' : ''; ?>">All Switches</a>
    <?php foreach ($switch_sub_categories as $sub_cat): ?>
        <a href="switches.php?filter=<?php echo urlencode($sub_cat); ?>" 
 ...
 <section class="product-section">
    <div class="product-container">
        <?php
            // ดึงข้อมูล Switches โดยใช้เงื่อนไข Filter
            $sql_switches = "SELECT * FROM products WHERE category = 'Switch' {$sql_condition} ORDER BY product_id ASC";
        ...
        ?>
                    <a href="product_detail.php?id=<?php echo $row['product_id']; ?>" class="product-card">
                        ...
                        <span class="price">฿<?php echo number_format($row['price'], 2); ?> / pcs</span> 
                    </a>
        <?php
                }
            } else {
                $message = "No Switches found";
