...
<head>
  ...
  <style>
    ...
    /* Style สำหรับไอคอนหมวดหมู่ */
    .category-icons {
        display: flex;
        justify-content: center;
        align-items: flex-start;
        gap: 40px; /* ระยะห่างระหว่างไอคอน */
        flex-wrap: wrap;
        margin-top: 30px;
        margin-bottom: 50px;
    }
    .category-link-card {
        ...
        align-items: center;
        width: 130px; /* ความกว้างของการ์ดไอคอน */
        text-align: center;
    ...
    .category-link-card img {
        width: 100px; /* ขนาดรูปไอคอน */
        height: 100px;
        object-fit: contain; /* ให้รูปอยู่ครบ ไม่โดนตัด */
        margin-bottom: 10px;
    }
    ...
    /* Style สำหรับ Product Grid */
    .product-grid-card {
        background: #fff; 
        border-radius: 15px; /* มุมมนขึ้น */
    ...
    .product-grid-card-body { 
        ...
        display: flex; /* ใช้ flex ช่วยจัด price ให้อยู่ล่างสุด */
        flex-direction: column;
    }
    ...
    .product-grid-card-price { font-weight: bold; color: #444; font-size: 1rem; margin-top: auto; /* ดันราคาลงล่างสุด */ }
...
<body>
  ...
  <div class="container">
      <div class="category-icons">
          <a href="keyboard.php" class="category-link-card">
              <img src="store/01_head.png" alt="Keyboards"> 
              <p>Keyboards</p>
          </a>
          <a href="switches.php" class="category-link-card">
              <img src="store/02_head.png" alt="Switches">
              <p>Switches</p>
          </a>
          <a href="keycap.php" class="category-link-card">
              <img src="store/03_head.png" alt="Keycaps">
              <p>Keycaps</p>
          </a>
          <a href="accessories.php" class="category-link-card">
              <img src="store/04_head.png" alt="Accessories">
              <p>Accessories</p>
          </a>
          <a href="modding.php" class="category-link-card">
  ...
  <div class="container my-5">
    ...
    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-4 mt-3"> 
      <?php
        // ดึงสินค้าทั้งหมด (อาจจะเพิ่ม LIMIT หรือ Pagination ทีหลัง)
        $sql_all = "SELECT * FROM products ORDER BY created_at DESC"; // เรียงจากใหม่ไปเก่า
      ...
      <?php 
          ...
          echo "<div class='col-12'><p class='no-products'>No products found.</p></div>";
        } // end if
        $conn->close(); 
      ?>
    </div> </div> <div class="container"><footer class="d-flex flex-wrap justify-content-between align-items-center py-3 my-4 border-top"> </footer></div>
